...
$Root = Join-Path $TempRoot ('erge-phase9-' + [guid]::NewGuid().ToString('N'))
...
function Invoke-RemoteSuccess {
    $output = & dotnet run --no-build --project $RemoteProject --configuration $Configuration -- --probe-screen 2>&1
    if ($LASTEXITCODE -ne 0) {
        throw "Remote provider screen probe failed. Output: $output"
    }
    $joined = $output | Out-String
    if ($joined -notmatch 'ERGE_REMOTE_PROVIDER_SCREEN_INFO_OK') {
        throw "Remote provider success marker missing. Output: $joined"
    }
    if ($joined -notmatch '"success":true') {
        throw "Remote provider response did not report success. Output: $joined"
    }
}

function Invoke-RemoteDenied {
    $output = & dotnet run --no-build --project $RemoteProject --configuration $Configuration -- --probe-screen 2>&1
    $code = $LASTEXITCODE
    $joined = $output | Out-String

    if ($code -eq 0) {
        throw "Remote provider unexpectedly succeeded. Output: $joined"
    }
    if ($joined -notmatch '"errorCode":"policy_denied"') {
        throw "Remote provider denial was not policy_denied. Output: $joined"
    }
}

function Invoke-LocalSuccess {
    $output = & dotnet run --no-build --project $LocalProject --configuration $Configuration -- --probe-screen 2>&1
    if ($LASTEXITCODE -ne 0) {
        throw "Local provider screen probe failed. Output: $output"
    }
    if (($output | Out-String) -notmatch 'ERGE_LOCAL_PROVIDER_SCREEN_INFO_OK') {
        throw "Local provider success marker missing. Output: $output"
    }
}
...
try {
    ...
    Write-Output 'ERGE_PHASE9_REMOTE_PROVIDER_E2E_OK'
}
catch {
